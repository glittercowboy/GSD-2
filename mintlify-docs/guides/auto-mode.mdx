...
For planning units (`plan-slice` / `refine-slice`), pre-execution checks validate plan consistency (file references, task inputs, dependencies); when these checks fail, a bounded planner-retry path applies:
...
2. If blocking failures continue after the retry budget is exhausted, auto mode pauses and surfaces escalation details plus the evidence path.
