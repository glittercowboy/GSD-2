...
describe("hook dispatch session workspace root", () => {
  test("dispatchHookUnit passes basePath explicitly to newSession and clears stale queue", async (t) => {
    ...
    assert.equal(clearQueueCalls, 1, "hook dispatch should clear stale queued events before newSession");
  });
});
