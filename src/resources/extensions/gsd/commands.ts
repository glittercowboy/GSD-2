...
export function registerGSDCommand(pi: ExtensionAPI): void {
  pi.registerCommand("gsd", {
    ...
    async handler(args: string, ctx: ExtensionCommandContext) {
      ...
      ctx.ui.notify(
        `Unknown: /gsd ${trimmed}. Use /gsd next|auto|stop|pause|status|visualize|queue|capture|triage|discuss|history|undo|skip <unit>|export|cleanup|prefs|config|hooks|doctor|migrate|remote|steer <change>|inspect|knowledge <type> <entry>.`,
        "warning",
      );
    },
  });
