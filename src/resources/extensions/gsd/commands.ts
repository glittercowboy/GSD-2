...
export function registerGSDCommand(pi: ExtensionAPI): void {
  pi.registerCommand("gsd", {
    description: "GSD — Get Shit Done: /gsd next|auto|stop|pause|status|queue|history|undo|skip|export|prefs|hooks|doctor|migrate|remote",

    getArgumentCompletions: (prefix: string) => {
      const subcommands = ["next", "auto", "stop", "pause", "status", "queue", "discuss", "history", "undo", "skip", "export", "prefs", "hooks", "doctor", "migrate", "remote", "cleanup"];
    ...
    async handler(args: string, ctx: ExtensionCommandContext) {
      ...
      ctx.ui.notify(
        `Unknown: /gsd ${trimmed}. Available: /gsd [next|auto|stop|pause|status|queue|history|undo|skip <unit>|export|cleanup branches|discuss|prefs|hooks|doctor|migrate|remote].`,
        "warning",
      );
    },
  });
