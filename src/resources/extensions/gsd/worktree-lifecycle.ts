...
function milestoneBranchHasImplementationCommits(
  basePath: string,
  milestoneId: string,
  branchName = autoWorktreeBranch(milestoneId),
): boolean {
  try {
    if (!nativeBranchExists(basePath, branchName)) return false;
    const mainBranch = nativeDetectMainBranch(basePath);
    return nativeCommitCountBetween(basePath, mainBranch, branchName) > 0;
  } catch {
    return false;
  }
}
...
function _mergeWorktreeModeImpl(
  ...
  try {
    ...
    if (!roadmapPath && !milestoneBranchHasImplementationCommits(originalBasePath, milestoneId)) {
      // No roadmap and no implementation commits — teardown but PRESERVE the
      // branch so commits are not orphaned (#1573).
    ...
    if (!roadmapPath) {
      notify(
        `Roadmap not found for ${milestoneId} but milestone branch has implementation commits — merging anyway.`,
        "info",
      );
    }

    const roadmapContent = roadmapPath ? readLifecycleFile(deps, roadmapPath) : "";
...
function _mergeBranchModeImpl(
  ...
  try {
    ...
    if (!roadmapPath && !milestoneBranchHasImplementationCommits(worktreeBasePath, milestoneId, milestoneBranch)) {
      debugLog("WorktreeLifecycle", {
        action: "mergeAndExit",
        milestoneId,
        mode: "branch",
